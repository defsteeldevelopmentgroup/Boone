...
class Stripe {
	 ...
	 */
	public function customer_create( $card, $email, $desc = NULL, $plan = NULL, $meta = NULL ) {
		$params['card'] = $card;
		$params['email'] = $email;
		if( $desc )
    ...
    public function retrieve_customer_from_event($stripe_data){
        //submit the object id to an array of different urls until a valid responce is received
        $target_urls = array();
        $test = $this->send_request($target_urls, $stripe_data);
        
        //return the stripe customer number
    }
